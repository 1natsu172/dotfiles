...
sandbox 有効時、**token を要する** npm / yarn / pnpm / bun 操作（private registry への install / publish 等。本リポジトリでは FLATT `npm.flatt.tech`）は、素のラッパー（`npm install`）ではなく **`fnox exec -- <pm> …` の形で明示的に打つ**。
