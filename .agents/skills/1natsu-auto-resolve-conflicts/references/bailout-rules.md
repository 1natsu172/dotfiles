...
**検出**:
- 同じ定義/領域の両側 diff を、**追加シンボルだけでなく既存行の意味変更も含めて**比較する。新規の関数・分岐・バリデーションの追加だけでなく、**既存コードの定数・演算子・戻り値・条件式・副作用の書き換え**（追加シンボルを伴わない意味変更）も「固有の変更」として扱う
- 単純な追記・削除や片側のみの変更ではなく、**両側それぞれに固有の変更**がある場合（追加・変更のいずれでも）
- 片側採用で消える変更（関数・分岐・定数・演算子・戻り値・検証・副作用・テスト・設定キー）を列挙し、それが意図的な廃止だと履歴から言えるか確認
- 比較は「追加された記号の有無」ではなく **変更されたハンク全体**を突き合わせること（`return x * 3` → `return x * 2` のような記号追加なしの意味変更を取りこぼさない）

**判断**: マーカーを消すだけなら片側採用で「できて」しまうが、それは**もう一方の変更を黙って捨てるデグレ**。両側が固有の変更をしているなら統合・移植が必須で、それが安全に行えないならビジネス判断の領域＝AI が独断する範疇を超える。**「片側採用すれば衝突は消える」を理由に片側採用してはならない。**

**Auto-OK 寄りの例外（厳格運用）**: 捨てる側が「純粋なリネーム / フォーマット / 空白・コメントのみ / 採用側の完全な部分集合（採用側が上位互換）」のいずれかで、固有の変更が無いと**積極的に証明できる**場合に限り片側採用してよい（Auto-OK パターン 2・3 参照）。証明の要件は **「捨てる側の各変更（新規追加〈関数・分岐・バリデーション・副作用〉だけでなく、既存行の意味変更〈定数・演算子・戻り値・条件・エラーハンドリング〉も含む）が、ひとつ残らず採用側にも存在する」ことを、変更ハンク全体を実際に diff で突き合わせて確認すること**（追加シンボルの grep だけで判断しない）。「採用側の方が大きいから上位互換だろう」という規模ベースの推測で片側採用してはならない（大きい側が、小さい側の固有の変更〈例: 戻り値の係数変更やエラーハンドリング〉を欠いていることは普通にある）。ひとつでも「採用側に無い固有変更」が見つかる、または網羅確認に確信が持てなければ、例外を適用せず **本ルール（Auto-No）に戻して bail-out** する。
