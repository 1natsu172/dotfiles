...
> 注: `--setting-sources` から `project` を外して起動すると、プロジェクトの rule は**ロードモードに関わらず全て skip される**（`paths` なしの起動時ロード、`paths` 付きの条件ロード、ネストした `.claude/rules/` のいずれも）。v2.1.220 で実測確認。setting source を絞る運用では、rule に書いた指示は効かない前提で組む。
...
> 注: プロジェクト `.claude/settings.json` は**起動ディレクトリのものだけ**読まれ、祖先からは継承されない（CLAUDE.md/rules の継承挙動とは別）。一方 repo root の `.claude/settings.local.json` は、v2.1.211 以降**起動ディレクトリに関わらずリポジトリ内の全セッションで読まれる**（v2.1.220 で実測確認）。サブディレクトリから起動する運用でも確実に効かせたい個人設定は local 側に置く。

## `paths` glob のマッチング仕様

rule の置き場所（ルート／ネスト）に依らず効く glob 一般の仕様。項目ごとに実測／未実測を明記する。

ブレース展開 `{a,b}` はグループの数だけパターン数が掛け算で増える（`src/*.{ts,tsx}` は2、`{a,b}/{c,d}/*.{ts,tsx}` は8）。1つの rule の `paths` 全体で**展開後1000パターン・4MiB の予算を共有**し、予算を超えるパターンは**展開されないまま**使われる（リテラルの `{}` として扱われ、何にもマッチしない）。ブレースを含まないパターンは予算を消費しない。パターン数の側は v2.1.220 で実測確認（`{a,b}` 9個はマッチ、10個はマッチせず）。4MiB の側は上流ドキュメント記載の仕様で、未実測。

glob の `[` は**ブラケット式**（`[abc]`）の開始として解釈され、ブラケット式として読めない `[` を含むパターン（例 `photos [2024/**`）はエラーにならず**何にもマッチしない**（同じ rule の他のパターンはそのまま効く）。リテラル `[` にマッチさせるには `\[` でエスケープする（例 `paths: ['photos \[2024/**']`、YAML では single quote で囲む）。v2.1.210 で実測準拠。

`paths` のマッチは、v2.1.198 以降 **symlink 経由でプロジェクトディレクトリへ到達したパスにも効く**（symlinked checkout 等）。上流ドキュメント記載の仕様で、未実測。
