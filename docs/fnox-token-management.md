...
- 中身は `fnox get <secret> --profile mcp --no-defaults` で**単一 secret だけ**を解決し `jq` でヘッダ JSON を
...
- **profile 分離**：MCP token を default `[secrets]` に置くと npm 等の `fnox exec`(default) が env 注入して
  しまうため専用 `mcp` profile に置き `--no-defaults` で他を混ぜない。ただし `--no-defaults` は op provider の
  認証 SA token（default 側）も解決対象から外すので、その参照を **profile 内にも同梱**する（値の実体は
  Keychain）。複数 MCP token も同 profile に並べ、`fnox get` が要求分だけ引く（接続毎の op fetch を1本に保つ）。
