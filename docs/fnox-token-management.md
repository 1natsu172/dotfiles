...
commit する。fnox docs も推奨）。fnox 1.25.1 では age provider の
`identity = { provider = "keychain", ... }` が未実装だったが、**1.26.0 (PR #515) で実装済み**。
現状はまだ 2 段（SA token を Keychain に直接）で運用しており、age 層の再導入は別タスク（それまで op
vault が source of truth）。なお age 層は **at-rest（commit する暗号文）の保護**であって、上述の
`env = false` による **env 注入制御とは別軸**（age で保管しても解決時は平文になり、env に載るか否かは
env フラグが決める）。
...
- age 3 段（`keychain → age → op`）の再導入は別タスク。fnox 1.26.0 (PR #515) で前提（age `identity` の
  provider 解決）は実装済み（上述「なぜ keychain 直結の暫定構成か」）。
