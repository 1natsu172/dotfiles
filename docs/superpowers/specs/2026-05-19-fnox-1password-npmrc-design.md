...
- 命名規約 `op_<vault スコープ>_<環境>`（例 `op_personal_develop`）により、将来
  `op_personal_production` / `op_work` 等を衝突なく追加可能
...
## D-2. fish の corepack エイリアスとの共存（重要）

`config.fish` には corepack 経由でパッケージマネージャを版管理するためのエイリアスがある:

```fish
alias npm="corepack npm"     # yarn/yarnpkg/pnpm/pnpx/npx も同様
```

fish では **function/alias が PATH より優先**されるため、この alias が PATH 上の
`fnox-shims/npm`（C の shim）を **shadow し、fnox exec を bypass** する。結果 fish では
token が注入されず `.npmrc` の `${FLATT_NPM_TOKEN}` が空展開 → **401**。
（bash/zsh には corepack alias が無いので shim が効き、この問題は出ない。
`which npm` は外部コマンドで fish function を無視し PATH を表示するため、
「which は fnox-shims を指すのに npm 実行は 401」という紛らわしい症状になる。）

**対処**: fish の corepack エイリアスを `fnox exec --` で包み、token 注入を復活させる:
...
これで fish は「alias → fnox exec（token 注入）→ corepack（版解決）→ 実 npm」、
bash/zsh は「fnox-shim（PATH）→ fnox exec → mise which → 実 npm」となり、
両系統で token が注入される。`bun`/`bunx` は corepack 対象外なので fish でも shim 経由。

注: corepack を mise の packageManager 対応（PR #8059 / mise 2026.2.8+）へ移行して
corepack 自体を廃止する選択肢もあるが、per-project packageManager 版解決の実挙動検証が
必要なため**別タスク**に分離する（本タスクは corepack 維持のまま token 注入を成立させる）。
...
3. SA トークンを macOS Keychain に登録:
   ...
   - 実際の SA token は macOS Keychain の service=`fnox`, key=`OP_SA_PERSONAL_DEVELOP` 配下に格納
...
1. **keychain → SA token 取得確認**:
   ```
   fnox get OP_SA_PERSONAL_DEVELOP
   ```
...
4. **env 隔離確認**:
   ...
   env | grep OP_SA_PERSONAL_DEVELOP      # 何も出ない
   ```
5. **npm 子プロセス内で展開されるか**:
   ```
...
- `op_<vault スコープ>_<環境>`（例 `op_personal_develop`）の命名規約により、後から
  `op_personal_production` / `op_work` / `op_readonly` 等を衝突なく追加可能
...
- 将来 fnox が age provider に keychain identity を実装したら:
  ...
  - `OP_SA_PERSONAL_DEVELOP` を `provider = "age"` 経由に切り替え（fnox.toml に age 暗号文として格納）
