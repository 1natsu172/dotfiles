...
| `~/.gnupg`（conf 以外） | 解放（agent 通信・署名検証に必要） | sandbox `allowWrite` で許可。ただし `gpg-agent.conf`/`gpg.conf`/`dirmngr.conf` は `denyWrite` | 署名運用・設定改ざん防止 |
...
- **ハードニング**: 署名で読むだけの `gpg-agent.conf` / `gpg.conf` / `dirmngr.conf` を `denyWrite` で個別保護する。`allowWrite: ~/.gnupg` を維持したまま、最大リスク（`gpg-agent.conf` の `pinentry-program` 書き換えによるパスフレーズ窃取）を塞ぐ。秘密鍵 `private-keys-v1.d` は別途三層遮断済みで流出経路はない。
